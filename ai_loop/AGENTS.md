...
| **Reviewer** | internal call | ✅ Active | Sanity check the diff and provide comments |
...
## Reviewer Logic
The Reviewer inspects the diff generated by the Coder and returns a dictionary
such as `{"approved": True, "comments": "Looks good"}`. If the diff is empty or
the Reviewer errors out, the pipeline defaults to an approval so execution never
halts.
